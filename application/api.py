    ...
    def decorator(*args, **kwargs):
        ...
        try:
            ...
            current_user = {"id": user.id, "username": user.username, "password": user.password, "email": user.email,
                            "followers_count": user.followers_count, "posts_count": user.posts_count}
        except:
...
class UserAPI(Resource):
    ...
    def put(self, username, current_user):
        ...
        if user and user.username != username:
            raise BusinessValidationError(status_code=400, error_code="BE1006",
...
class SearchUserAPI(Resource):
    ...
    def get(self, query, current_user):
        ...
        for user in users:
            result.append(
                {"id": user.id, "username": user.username, "email": user.email, "followers_count": user.followers_count,
...
class PostAPI(Resource):
    ...
    def post(self, current_user):
        ...
        post = Post(title=data['title'], description=data['description'], timestamp=datetime.utcnow(), image=image,
    ...
    def put(self, current_user):
        ...
        if post.user_id != current_user["id"]:
            raise BusinessValidationError(status_code=401, error_code="BE105",
                                          error_message="Not allowed to change other's post!")
    ...
    def delete(self, post_id, current_user):
        ...
        if post.user_id != current_user["id"]:
            raise BusinessValidationError(status_code=401, error_code="BE105",
                                          error_message="Not allowed to delete other's post!")
...
class AllPostsAPI(Resource):
    ...
    def get(self, user_id, current_user):
        ...
        for post in posts:
            result.append({"id": post.id, "title": post.title, "description": post.description,
                           "timestamp": post.timestamp.isoformat(),
                           "image": base64.b64encode(post.image).decode('utf-8'), "likes_count": post.likes_count})
...
class FollowingAPI(Resource):
    ...
    def get(self, user_id, current_user):
        ...
        if user_id == current_user["id"]:
            raise BusinessValidationError(status_code=404, error_code="BE107",
                                          error_message="Cannot follow/unfollow to self!")

        following = Following.query.filter_by(follower_id=current_user["id"], followed_id=user_id).first()

        if following:
            raise BusinessValidationError(status_code=404, error_code="BE108",
